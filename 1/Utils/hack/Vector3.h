...
struct Vector3 {
    ...
    Vector3(const VInt3& v) : x(static_cast<float>(v.X) / 1000.0f), y(static_cast<float>(v.Y) / 1000.0f), z(static_cast<float>(v.Z) / 1000.0f) {}
};
...
static float NormalizeAngele(float a){
    while (a > 360)
        a -= 360;
    while (a < 0)
        a += 360;
